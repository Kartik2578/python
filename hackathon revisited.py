...
class Fee:
    ...
    def fine(self):
        ...
        if difference <= 1:
            ...
            fine = ((difference-1)*self.amount )*1.1
            totalFee = self.amount + fine
            print(print(">> Fee Not Paid for {} months is{}".format(difference , totalFee)))
            self.amount = totalFee
...
def main():
    ...
    if choice == 1:
        ...
        fee = Fee()
        fee.inputFeeDetails()
        i = int(input("enter 1 if its 1st entry for customer and u want to save it "))
        if i == 1:
            data = fee.toCSV()
